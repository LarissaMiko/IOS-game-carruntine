...
class MultiplayerView: SKScene {
    ...
    init (givenController: MultiplayerController, size: CGSize){
        super.init(size: size)
        multiplayerController = givenController
    ...
    func createLayers(){
        ...
        layerNode = scaling.load(layerFile: "Multiplayer", menuLayer: menuLayer)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let location = touch?.location(in: self) {
            ...
            if nodeArray.first?.name == "multiplayerBackButton" {
                multiplayerController.backButtonPressed()
            }
            else if nodeArray.first?.name == "hostButton" {
                multiplayerController.hostButtonPressed()
            }
        }
    }
}
